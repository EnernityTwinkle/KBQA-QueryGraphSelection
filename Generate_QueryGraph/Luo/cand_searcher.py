...
from combinator import predicate_filtering
from fb_helper import get_pred_name, inverse_predicate, get_begin_dt_pred, \
...
class CandidateSearcher:
    ...
    def coarse_search(self, path_len, entity_linkings, conflict_matrix,
        ...
        fuzzy_query = self.build_fuzzy_query(path_len, entity_linkings, cur_comb)   # 构建mid对应的实体的所有关系候选
        if self.vb >= 1:
    ...
    def find_typed_schemas(self, type_linkings, conflict_matrix, start_schema, visit_arr, simple_type_match):
        ...
        if not simple_type_match:           # perform type filtering
            use_type_linkings = self.type_filtering(start_schema, type_linkings)#  过滤掉那些答案类型不在候选关系类型的查询图
    ...
    def build_fuzzy_query(path_len, entity_linkings, cur_comb):
        '''
        功能：根据cur_comb中的实体构建主路径搜索的sparql语句
        存在的问题：没有反向的搜索路径
        '''
        ...
        sparql_str = ' '.join(sparql_lines)  # 构建查询谓词p的sparql语句
        # import pdb; pdb.set_trace()
        return sparql_str
