...
from fb_helper import get_domain, get_range, get_item_name,\
...
class CompqSchema(object):
    ...
    def serialize(self):
        print('main_pred_seq:', self.main_pred_seq, 'var_types:', self.var_types, 'var_time_preds:', self.var_time_preds,
        'var_ord_preds:', self.var_ord_preds, 'raw_paths:', self.raw_paths)
    @staticmethod
    ...
    def construct_path_list(self):
        ...
        if self.placeholder_policy == 'ActiveDT':
            """
            将time类型的shcema数据存入replace_linkings.
    ...
    def infer_var_types(self):      # given the main predicate, infer the explicit types of o1 and o2
        '''
        功能：获取主路径中的o1、o2节点对应的类型，通过关系得到的
        '''
    ...
    def infer_time_preds(self):     # gien the explicit types of each variable, produce its time-related predicates
        ...
        print('self.var_time_preds:', self.var_time_preds)
        # import pdb; pdb.set_trace()
    ...
    def build_sparql(self, simple_time_match, ans_type=''):
        """
        ...
        for cate, link_data, pred_seq in self.raw_paths:
            ...
            if cate == 'Main':
                ...
                if simple_time_match:
                    ...
                    tm_end_pred = get_end_dt_pred(tm_begin_pred)        # try finding a time intervalm
                attach_idx = self.hops if len(pred_seq) == 1 else 1
        ...
        if range_type in ('type.int', 'type.float', 'type.datetime'):
            ...
            if(ans_type != ''):
                var_symbol_list += ['?type%d' % self.hops] 
                where_lines.append('OPTIONAL { ?o%d fb:%s ?type%d .}' % (self.hops, ans_type, self.hops))
                # print('ans_type:', ans_type, var_symbol_list)
        else:
