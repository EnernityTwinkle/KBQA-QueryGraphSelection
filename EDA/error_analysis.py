...
# 分析系统出错的情况
def system_error_instance(qid2maxf1, qid2maxcand, qid2f1, qid2onecand):
    qid2diff = {}
    for qid in qid2maxf1:
        if(qid2f1[qid] < 0.1):
            qid2diff[qid] = (qid2maxcand[qid], qid2onecand[qid])
...
def rerank_false(qid2f1_true, qid2onecand_true, f):
...
def webq_error_analysis():
    file_name = '/home/chenwenliang/jiayonghui/gitlab/rerankinglab/runnings/model/webq/5595_rerank_listwise_negOrder_pos10_baseline_1score_webq_neg_30_42_100/prediction'
    scores = read_prediction_scores(file_name)
    file_name = BASE_DIR + '/runnings/train_data/webq/t_bert_top20_from5530.txt'
    ...
    # f = open('webq_rerank_false.txt', 'w', encoding='utf-8')
    # rerank_false(qid2f1, qid2onecand, f)
    # import pdb; pdb.set_trace()
    # 获取qid与问句之间的对应关系
    # qa_list = load_compq()
    qa_list = load_webq()
    ...
    qid2diff = system_error_instance(qid2maxf1, qid2maxcand, qid2f1, qid2onecand)
    fout = open('./重排序系统出错的问题.txt', 'w', encoding='utf-8')
    for qid in qid2diff:
