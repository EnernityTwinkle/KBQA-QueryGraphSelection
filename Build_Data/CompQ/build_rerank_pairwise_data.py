...
def write2fileTrain(file_name: str, qid2cands: Dict[str, List[List[Tuple[List[str], int]]]]) -> None:
    f = open(file_name, 'w', encoding='utf-8')
    for qid in qid2cands:
        num = 0
        for candPos in qid2cands[qid][1]:
            for candNeg in qid2cands[qid][0]:
                # import pdb; pdb.set_trace()
                f.write('\t'.join(candPos[0]) + '\n')
                f.write('\t'.join(candNeg[0]) + '\n')
                num += 1
    f.flush()
