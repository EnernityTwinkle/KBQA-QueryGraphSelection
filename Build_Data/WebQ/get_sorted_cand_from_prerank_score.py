...
# 根据得分将所有候选进行排序并输出
def sort_cands(scores_file_name, data_file_name, dataType, qidPos = -1):
    ...
    while i < len(lines):
        line = lines[i]
        qid_temp = line.strip().split('\t')[qidPos]
        if(qid_temp != qid):
            qid = qid_temp
            if(begin == -1):
                begin = i
...
if __name__ == "__main__":
    ...
    ######################################################
    sort_cands(BASE_DIR + '/runnings/model/webq/5595_rerank_listwise_negOrder_pos10_baseline_1score_webq_neg_30_42_100/prediction_test_rerank', 
                BASE_DIR + '/runnings/train_data/webq/t_bert_top20_from5530.txt', 'test_fromrerank', -2)

    # sort_cands(BASE_DIR + '/runnings/model/webq/5595_rerank_listwise_negOrder_pos10_baseline_1score_webq_neg_30_42_100/prediction_dev_rerank', 
    #             BASE_DIR + '/runnings/train_data/webq/webq_with_answer_info_dev_all.txt', 'dev_fromrerank', -1)

    # sort_cands(BASE_DIR + '/runnings/model/webq/5595_rerank_listwise_negOrder_pos10_baseline_1score_webq_neg_30_42_100/prediction_train_rerank', 
    #             BASE_DIR + '/runnings/train_data/webq/webq_with_answer_info_train_all.txt', 'train_fromrerank', -1)
