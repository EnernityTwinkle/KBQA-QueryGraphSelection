...
import logging
import urllib
import requests  # urllib.parse必须要包
# import urllib2
import time
...
class CoreNLPParser(object):
    ...
    def __init__(self, host):
        self.host = host
        # self.server = jsonrpclib.Server(host)
    '''
    @staticmethod
    ...
    def parse(self, query, include_json=False,
        ...
        :return:
        """
        # logger.debug(u"Parsing '{0}'.".format(intention))
        t0 = time.time()
        value = {"text": query}
...
def sort_dep_edge_word(dependences_list):
    tmp_dic = {}
    for edge in dependences_list:
        child = []
        parents = []
        edge = edge.strip().split(",")
        for next_edge in dependences_list:
            next_edge = next_edge.strip().split(",")
            if edge == next_edge:
                continue
            else:
                if edge[2] == next_edge[1]:
                    child.append(next_edge[0])
                elif edge[1] == next_edge[2]:
                    parents.append(next_edge[0])
        if edge[0] not in tmp_dic.keys():
            tmp_dic[edge[0]] = [child, parents]
        else:
            tmp_dic[edge[0]].append(child)
            tmp_dic[edge[0]].append(parents)
    return tmp_dic

"""pjzhang Produce function"""
def sort_dep_word(co_word):
    tmp_dic = {}
    for i in co_word:
        ...
        try:
            ...
            return ['sssss']
        tmp_dic[i] = idx
    a1 = sorted(tmp_dic.items(), key = lambda x:x[1], reverse = False)
    fin_word = [i for i,j in a1]
    return fin_word

# 产生输入模型的[heads, dep_words]词典 {0：[-1,'root']···}
def produce_model_input(dependences_list):
    model_input_dict = {}
    for idx,i in enumerate(dependences_list):       
        i = i.split(",")
        if "-" in i[2].replace('"',"")[-2:]:
            word_idx = int(i[2].replace('"',"")[-1])-1
        else:
            word_idx = int(i[2].replace('"',"")[-2:])-1
        if "-" in i[1].replace('"',"")[-2:]:
            head_idx = int(i[1].replace('"',"")[-1])-1
        else:
            head_idx = int(i[1].replace('"',"")[-2:])-1            
        dep_word = i[0].replace('"',"")
        model_input_dict[word_idx] = [head_idx, dep_word]       
...
def parse_json_result(json_response, include_json=False,
    ...
    if include_json:
        parse_result.json_result = json_response
...
def main():
    for query in ['where are the nfl redskins from?', 'who is the prime minister of ethiopia?', 'what is the name of justin bieber brother?']:
        # import pdb;pdb.set_trace()
        # parser = CoreNLPParser("http://localhost:10001/parse")
        parser = CoreNLPParser("http://192.168.126.139:4000/parse")
