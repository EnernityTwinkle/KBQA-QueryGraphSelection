...
from fb_helper import load_type_name, is_type_ignored
...
class LinkData:
    ...
    def __init__(self, category, start, end, mention, comp, value, name, link_feat, gl_pos=None):
        ...
        self.start = start
        self.end = end
        self.mention = mention  # 候选实体字符串
        self.comp = comp
        self.value = str(value)  # 实体id
        self.name = name  # 实体字符串
...
class GlobalLinker:
    ...
    def __init__(self, wd_emb_util, q_links_fp, ordinal_fp='../../data/fb_metadata/ordinal_fengli.tsv'):
        ...
        self.ordinal_word_dict = {}         # <ordinal word, its corresponding number> {'first': 1, '1st': 1, 'most': 1, 'maximum': 1, 'maximal': 1, 'second': 2, '2nd': 2, 'third': 3···}
        self.superlative_words = set([])        # contain both -est words and xx-th words{'eldest', 'tallest', 'richest', 'minimum', 'farthest', 'simplest'···}`
        """加载所有序数词对"""
        ...
        """加载yih smart结果, cate=entity"""
        LogInfo.begin_track('Load entity linking table from [%s]:', q_links_fp)
        # line:0000\t5\t7\tjustin bieber\tm.06w2sn5\tJustin_Bieber\t{"score": 62742.409519802866, "source": 0}
    ...
    def perform_linking(self, q_idx, tok_list, entity_only=False):
        tok_list = list(map(lambda x: x.lower(), tok_list))       # all steps ignore cases.
        Tt.start('entity')
        ...
        for st in range(tok_size):           
            for ed in range(st+1, tok_size+1):
                ...
                if phrase_emb is None:
                    continue
                for tp, (type_name, type_emb) in self.type_emb_dict.items():  # 遍历所有的关系名称，与当前的关系tri-grams词进行计算相似度
