...
from src.utils.data_processing import readTrainFile, readQue2AnswerFromTestset, readTrainFileForSparql
...
def transQueryGraph2SeqItemsForSystem(graphInfo, normElement):
    mainPathSeq = getPathTriplesSubPath(graphInfo)
    entityConstrainKey = getEntityConstrainSubPath(graphInfo)
    virtualEntityConstrainKey = getVirtualEntityConstrainSubPath(graphInfo)
    higherOrderConstrainKey = getHigherOrderConstrainSubPath(graphInfo)
...
def data2json(inputPath: str) -> Dict[str, str]:
    ...
    with open(inputPath, "r") as fin:    # 这样可以一起写
        ...
        for idx, i in enumerate(ques):
            # if('官网是"http://im.qq.com/"的软件支持哪些语言' in ques[idx]):
            #     import pdb; pdb.set_trace()
            pos = ques[idx].find(':')
            que = ques[idx][pos + 1:]
            # que = ques[idx].split(":")[-1]
...
def generateTrainSeqNoSparql(normElement):
    ...
    for i, que in enumerate(que2cands):
        ...
        for cand in que2cands[que]:
            ...
            newGoldAnswer = processAnswerEntity(goldAnswer)
            predictionAnswer = processAnswerEntity(predictionAnswer)
            p, r, f1 = f1Item(newGoldAnswer, predictionAnswer)
            if(f1 < 0.00001):
                newGoldAnswer = processAnswerValue(goldAnswer)
                p, r, f1 = f1Item(newGoldAnswer, predictionAnswer)
...
def generateTrainSeqNoSparqlByItems(normElement, dataType):
    if(dataType == 'train'):
        ...
        fileName = BASE_DIR + '/dataset/ccks2021/testset_zhengqiu.json'
        que2answer = readQue2AnswerFromTestset(fileName)
        candsFile = BASE_DIR + '/data/candidates/8677_testset_ccks2021_withAnswerType_filter_rel_top2000_query_graph_cands_0307_expand10_add_100_1000_10000_100000_1000.txt'
        que2cands = readCandsInfo(candsFile)
        seqsFile = BASE_DIR + '/data/candidates/seq/testset_seq_8677.txt'
    que2seqs = {}
    for i, que in enumerate(que2cands):
        tempDic = {}
        goldAnswer = que2answer[que]
...
def generateTrainSeqForSystem(queryGraphFile):
    normElement = False
    que2cands = readCandsInfo(queryGraphFile)
    seqsFile = BASE_DIR + '/src/rerank/system/temp/seq_system.txt'
    que2seqs = {}
    for i, que in enumerate(que2cands):
        tempDic = {}
        # goldAnswer = que2answer[que]
        ...
        for cand in que2cands[que]:
            ...
            # if(f1 < 0.00001):
                ...
                # f1 = f1Item(newGoldAnswer, predictionAnswer)
            # candSeq = transQueryGraph2Seq(cand, normElement)
            candSeq = transQueryGraph2SeqItemsForSystem(cand, normElement)
            # print(candSeq)
            f1 = -1
            candSeq["f1"] = f1
            if(f1 >= 0.5):
...
def generateTrainSeqSparqlByItems(normElement, dataType):
    ...
    if(dataType == 'train'):
        fileName = BASE_DIR + '/dataset/ccks2021/ccks2021_task13_train.txt'
        # candsFile = BASE_DIR + '/data/candidates/9604_stagg_train_ccks2021_filter_rel_top30000_query_graph_cands_expand10_add_100_1000_10000_100000_1000.txt'
        candsFile = BASE_DIR + '/data/candidates/9750_4hop4_train_ccks2021_withAnswerType_filter_rel_top300_query_graph_cands_0307_expand10_add_100_1000_10000_100000_1000.txt'
        # seqsFile = BASE_DIR + '/data/candidates/seq/stagg_trainset_seq_9604.txt'
        seqsFile = BASE_DIR + '/data/candidates/seq/9750_trainset_withAnswerType_seq.txt'
    ...
    for i, que in enumerate(que2cands):
        ...
        if(que not in que2trueNum):
            que2trueNum[que] = 0
        cands2f1 = {}
        # if(que != '官网是"http://im.qq.com/"的软件支持哪些语言？'):
        #     continue
        for cand in que2cands[que]:
            key = getKey(cand)
            if(key in tempDic):
...
def generateSeqNoSparqlByItemsForMultiTypes(normElement, dataType):
    if(dataType == 'train'):
        fileName = BASE_DIR + '/dataset/ccks2021/ccks2021_task13_train.txt'
        # candsFile = BASE_DIR + '/data/candidates/9535_0317_new_ex_ccks_trainset_top300_100_1000_10000_100000_1000.txt'
        # seqsFile = BASE_DIR + '/data/candidates/seq/trainset_seq_9535_multiTypes.txt'
        candsFile = BASE_DIR + '/data/candidates/8696_0317new_ex_ccks_STAGG_trainset_top30000_100_1000_10000_100000_1000.txt'
        seqsFile = BASE_DIR + '/data/candidates/seq/stagg_trainset_seq_8696_multiTypes.txt'
        que2goldTriples = json2elements2(fileName, BASE_DIR + '/src/querygraph2seq/elements.txt')
        # import pdb; pdb.set_trace()
        que2answer = readTrainFile(fileName)
        que2cands = readCandsInfo(candsFile)
...
def generateSeqNoSparqlByItemsForMultiTypesEval(normElement, dataType):
    if(dataType == 'dev'):
        fileName = BASE_DIR + '/dataset/ccks2021/ccks2021_task13_train.txt'
        ...
        # candsFile = BASE_DIR + '/data/candidates/9444_0317_new_ex_ccks_devset_top300_100_1000_10000_100000_1000.txt'
        # seqsFile = BASE_DIR + '/data/candidates/seq/devset_seq_9444_multiTypes.txt'
        candsFile = BASE_DIR + '/data/candidates/8408_0317new_ex_ccks_STAGG_devset_top30000_100_1000_10000_100000_1000.txt'
        seqsFile = BASE_DIR + '/data/candidates/seq/stagg_devset_seq_8408_multiTypes.txt'
        que2answer = readTrainFile(fileName)
        que2cands = readCandsInfo(candsFile)
    elif(dataType == 'test'):
        fileName = BASE_DIR + '/dataset/ccks2021/testset_zhengqiu.json'
        que2answer = readQue2AnswerFromTestset(fileName)
        candsFile = BASE_DIR + '/data/candidates/7291_0317new_ex_ccks_STAGG_testset_top30000_100_1000_10000_100000_1000.txt'
        seqsFile = BASE_DIR + '/data/candidates/seq/stagg_testset_seq_7291_multiTypes.txt' # 输出文件的路径
        que2cands = readCandsInfo(candsFile)
...
if __name__ == "__main__":
    ...
    generateSeqNoSparqlByItemsForMultiTypesEval(normElement, dataType = 'test')
