...
def selectionErrorTest():
    ...
    # dir_name = '/home/chenwenliang/jiayonghui/ckbqa/src/model_train/models/7372_pointwise_ccks_comp_bert_trainexamples_devneg50_seq150_CE_neg15_24_42_1'
    dir_name = '/home/chenwenliang/jiayonghui/ckbqa/src/model_train/models/7490_ccks_comp_listwise_bert_devneg100_seq150_CE_group_20_1_42_100'
    # dir_name = '/home/chenwenliang/jiayonghui/ckbqa/src/rerank/model_train/models/7498_rerank_ccks_comp_listwise_bert_negOrder_answerType_seq150_CE_group_30_1_42_100'
    file_name = dir_name + '/prediction_test'
    scores = read_prediction_scores(file_name)
    # file_name = dir_name + '/prediction_test128\log'
    scoreLog = readScoreLog(file_name)
    file_name = BASE_DIR + '/data/train_data/ccks_comp/testset_model_data.txt'
    # file_name = BASE_DIR + '/data/train_data/ccks_comp/rerank_t_bert_top40.txt'
    ...
    # import pdb; pdb.set_trace()
    qid2diff = sort_error_instance(qid2maxf1, qid2maxcand, qid2f1, qid2onecand)
    fout = open('./ranking_test_selection_error_100.txt', 'w', encoding='utf-8')
    ids = [i for i in range(len(qid2diff))]
    import random
    random.shuffle(ids)
    selectIds = ids[0:100]
    # import pdb; pdb.set_trace()
    for i, qid in enumerate(qid2diff):
    ...
    # import pdb; pdb.set_trace()
    qid2true = sort_true_instance(qid2maxf1, qid2maxcand, qid2f1, qid2onecand)
    fout = open('./querygraaph_generation_test_selection_true.txt', 'w', encoding='utf-8')
    for qid in qid2true:
        # import pdb; pdb.set_trace()
        if(type(qid2true[qid][0][1][1]) is not str):
            qid2true[qid][0][1][1] = json.dumps(qid2true[qid][0][1][1], ensure_ascii=False)
