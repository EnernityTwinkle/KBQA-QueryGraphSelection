...
def cal_f1(file_name1, file_name2, data_type, actual_num = 0, log=0):
    ...
    while i < len(lines):
        ...
        if(end != -1):
            ...
            # import pdb; pdb.set_trace()
            if(have_p_r):
                p += float(eval(eval(lines[begin + num].strip())[-1])['p'])
                r += float(eval(eval(lines[begin + num].strip())[-1])['r'])
            if(log == 1):
                print(p, r, f1)
            # if(f1 > 0):
            #     print(begin+num, f1)
...
if __name__ == "__main__":
    exit()
    file_name1 = '/home/chenwenliang/jiayonghui/ckbqa/src/model_train/models/7357_ccks_comp_listwise_bert_randomseq_devneg100_seq150_CE_group_15_1_42_100/prediction'
    file_name2 = BASE_DIR + '/data/train_data/ccks_comp/testset_model_data.txt'
    cal_f1(file_name1, file_name2, 't', actual_num=1397)