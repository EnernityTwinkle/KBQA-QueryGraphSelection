...
time_2 = re.compile(r'\d{4}[年]\d{1,2}[月]')
time_1 = re.compile(r'\d{4}[年]\d{1,2}[月]\d{1,2}[日]')
# 价格正则
...
def readQuestionType(fileName:str) -> Dict[str, str]:
    with open(fileName, 'r', encoding='utf-8') as fread:
        ...
        for line in fread:
            ...
            if(lineCut[0] not in que2type):
                que2type[lineCut[0]] = lineCut[1]
        return que2type
...
def find_time_constrain(question):
    res = re.findall(time_1, question)
    # import pdb; pdb.set_trace()
    if not res:
        res = re.findall(time_2, question)
    if not res:
        res = re.findall(time_3, question)
    if not res:
        res = re.findall(time_4, question)
        # if res:
        #     return res[0]
    if not res:
        return ''
    ...
    return (res, begin, begin + len(res) - 1)
...
def addMentionAndDigitToLinking(que2entitys: Dict[str, List[str]]):
    for que in que2entitys:
        ...
        time = find_time_constrain(que)
        if(len(time) > 0):
            timeStr = str(time)
            if(timeStr not in que2entitys[que]):
                que2entitys[que][timeStr] = [time[0]]
            # import pdb; pdb.set_trace()
        ...
        # price_constrain = find_price_constrain(que)
        # distance_contrain = find_distrance_constrain(que)
        # import pdb; pdb.set_trace()
    return que2entitys
