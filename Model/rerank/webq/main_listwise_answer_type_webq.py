...
def test(best_model_dir_name, fout_res, args):
    ...
    for input_ids, input_mask, segment_ids, label_ids, rels_ids in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for item in logits:
            f_valid.write(str(float(item)) + '\n')
    f_valid.flush()
    p, r, F_dev = cal_f1_with_position(file_name1, args.data_dir + args.t_file_name, 't', -4)
    fout_res.write(str(p) + '\t' + str(r) + '\t' + str(F_dev) + '\n')
    fout_res.flush()
...
if __name__ == "__main__":
    ...
    # for N in [100, 80, 60, 50, 40, 30, 20]:
        ...
        parser.add_argument("--output_dir",default=BASE_DIR + '/runnings/model/webq/rerank2_listwise_with_answer_type_webq_neg_' + str(N) + '_' + str(seed) + '_' + str(steps) + '/',type=str)
        ...
        parser.add_argument("--v_file_name",default='v_bert_top10' + '_from5636.txt',type=str)
        parser.add_argument("--t_file_name",default='t_bert_top10' + '_from5636.txt',type=str)
        parser.add_argument("--T_file_name",default='webq_rerank2_T_bert_negOrder_pos10_1_n_top' + str(N) + '.txt',type=str)
        ...
        # best_model_dir_name = '/home/chenwenliang/jiayonghui/gitlab/rerankinglab/runnings/model/webq/rerank2_listwise_with_answer_type_webq_neg_40_42_100/0.972227199426626_0.5727290187658267_4/'
