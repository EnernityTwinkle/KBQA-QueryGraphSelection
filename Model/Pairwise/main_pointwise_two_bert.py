...
from Model.Pairwise.BertEncoderX import BertForTwoSequence, BertForTwoSequenceCosine, TwoBertForTwoSequence
...
def main(fout_res, args: ArgumentParser):
    ...
    model = TwoBertForTwoSequence.from_pretrained(args.bert_model,cache_dir=cache_dir,num_labels=1)
    model.to(device)
    # Prepare optimizer
...
def test(best_model_dir_name, fout_res, args):
    ...
    cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE))
    model = TwoBertForTwoSequence.from_pretrained(best_model_dir_name,cache_dir=cache_dir,num_labels=1)
    model.to(device)
    # 构建验证集数据  
...
if __name__ == "__main__":
    ...
    for N in [5]:
        ...
        parser.add_argument("--output_dir",default=BASE_DIR + '/runnings/model/webq/test_bert_webq_pointwise_two_bert_neg_' + str(N) + '_' + str(seed) + '_' + str(steps) + '/',type=str)
