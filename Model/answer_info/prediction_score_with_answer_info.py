...
from Model.common.BertEncoderX import BertFor3PairSequenceWithAnswer
...
def main(fout_res, args: ArgumentParser):
    ...
    # import pdb; pdb.set_trace()   
    eval_data = processor.convert_examples_to_features_with_answer_info(eval_examples, tokenizer)
    ...
    if args.do_train:   
        i_train_step = 0
        train_data = processor.convert_examples_to_features_with_answer_info(train_examples, tokenizer)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if "classification" in merge_mode:
                    # *******************交叉熵损失函数*********************
                    label_ids = label_ids.view(-1,3)[:,0]
...
def test(best_model_dir_name, fout_res, args):
    ...
    cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE))
    model = BertFor3PairSequenceWithAnswer.from_pretrained(best_model_dir_name,cache_dir=cache_dir,num_labels=2)
    model.to(device)
    # 构建验证集数据  
    ...
    eval_data = processor.build_data_for_model(eval_data, tokenizer, device)
    # import pdb; pdb.set_trace()
    file_name1 = args.output_dir + 'prediction_test_repreidct'
    f_valid = open(file_name1, 'w', encoding='utf-8')
    # Run prediction for full data
    eval_sampler = SequentialSampler(eval_data)
    eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
...
def dev(best_model_dir_name, fout_res, args):
    ...
    eval_data = processor.build_data_for_model(eval_data, tokenizer, device)
    # import pdb; pdb.set_trace()
    file_name1 = args.output_dir + 'prediction_dev_repredict'
    f_valid = open(file_name1, 'w', encoding='utf-8')
    # Run prediction for full data
    eval_sampler = SequentialSampler(eval_data)
    eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
...
if __name__ == "__main__":
    ...
    for N in [10]:
        ...
        parser.add_argument("--v_file_name",default='v_bert_top' + str(N) + '_from5244.txt',type=str)
        ...
        parser.add_argument("--t_file_name",default='t_bert_top' + str(N) + '_from5244.txt',type=str)
        ...
        fout_res = open(args.output_dir + 'result_predict_x.log', 'w', encoding='utf-8')
        # import pdb; pdb.set_trace()
        # best_model_dir_name = main(fout_res, args)
        best_model_dir_name = '/home/jiayonghui/github/sum/RankingQueryGraphs/runnings/model/webq/answer_info_rerank_3bert2bert_webq_pointwise_to2add_5epoch_neg_10_42_50/0.9748344370860927_0.594810476881947_3'
